...
#_(defsc Pic [this {data :data :pic/keys [id] :as props}]
  {:query               [:pic/id :data {[:data/id :pic/id] (comp/get-query Data)}]
   :route-segment       ["pic" :pic/id]
   :will-enter          (fn [app params]
                          (log/info "Pic will enter, params:" params)
                          (dr/route-immediate [:pic/id (js/parseInt (:pic/id params))]))
   :initial-state       {:data {}}
   :ident               :pic/id}
  (dom/div "picture:" id data "props nil?" (nil? props) (str props) ))
(defsc Pic [this { :pic/keys [id] :as props}]
  {:query               [:pic/id ]
   :route-segment       ["pic" :pic/id]
   :will-enter          (fn [app params]
                          (log/info "Pic will enter, params:" params)
  ...
  (dom/div
   (dom/div "picture:" id)
   (dom/div (str "props nil?" (nil? props) "//" props))))

#_(defsc Txt [this {:data/keys [id label pic txt] :as props}]
  {:query               [:data/id :data/label :data/pic :data/txt]
...
(defsc BigDetail [this {:data/keys [id label pic txt] :as props router :xi/router}]
  {:query               [:data/id :data/label :data/pic :data/txt {:xi/router (comp/get-query TabRouter)}]
   :route-segment       ["d" :data/id]
   :will-enter          (fn [app params]
                          (log/info "BigDetail will enter, params:" params)
                          (dr/route-immediate [:data/id (js/parseInt (:data/id params))]))
   :initial-state       {:xi/router {}}
   :ident               :data/id}
           ...
           (let [current-tab (some-> (dr/current-route this this) first keyword)]
             (dom/div :.ui.container
                      (dom/div :.ui.secondary.pointing.menu
                               ...
                               #_(dom/a :.item {:classes [(when (= :txt current-tab) "active")]
